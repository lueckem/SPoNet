...
def _(np):
    ...
    r = np.array([[0, 0.8, 0.2], [0.2, 0, 0.9], [0.8, 0.3, 0]])
    r_tilde = 0.01 * np.array([[0, 0.9, 0.8], [0.7, 0, 0.9], [0.9, 0.7, 0]])
...
def _(network, num_opinions, r, r_tilde):
    ...
    params = CNVMParameters(
        num_opinions=num_opinions, network=network, r=r, r_tilde=r_tilde, alpha=1
    )
    return CNVM, params
...
def _(CNVM, np, num_agents, params):
    ...
    x_init = np.concatenate(
        [
            0 * np.ones(int(20 * num_agents / 100)),
